...
class Board:
    ...
    def _heuristic(self):
        ...
        opp_colour = PlayerColor.RED if curr_colour == PlayerColor.BLUE else PlayerColor.BLUE
        curr_player_cells = dict(filter(lambda item: item[1].color == curr_colour, self.state.items()))
        opp_cells = dict(filter(lambda item: item[1].color == opp_colour, self.state.items()))

        num_player_cells = len(curr_player_cells)
        num_opp_cells = len(opp_cells)

        # heuristic that weights states with more of your pieces higher
        value += (num_player_cells - num_opp_cells) / 2.0

        safe_pieces = [x for x in curr_player_cells.items() if not self._is_under_attack(x)]

        value += len(safe_pieces) / 12.0
        # randomises
